...
class AcquisitionFunctionEI:
    ...
    def __call__(self, x):
        ...
        if self.invert:
            diff = (self.best_val + self.xi)  - mean
        else:
            diff = mean - (self.best_val + self.xi)


        ei = diff * spstat.norm.cdf(diff / std) + std * spstat.norm.pdf(diff / std) *self.explore
        ei = ei * efficiency
...
class BasicOptimiser:
    ...
    * explore: turns off exploration by changing aquisition function to mimic
    ...
    def __init__(self, n_dims, mean_cutoff=None, *,
            ...
            scale=None, use_efficiency=True,xi =0.01, explore = True,
            normalise_Y = False, **kwargs):
        self.n_dims = n_dims
        if kernel is None:
        ...
        self.x_samples = []
        self.y_samples = []
        self.y_samples_unnorm = []
        self.y_err_samples = []
        self.y_err_samples_unnorm = []
        self.mean_cutoff = mean_cutoff
    ...
    def tell(self, x, y, y_error):
        ...
        self.y_samples_unnorm.append(y)
        self.y_err_samples_unnorm.append(y_error)
        if self.normalise_Y:
            if len(self.y_samples_unnorm) >1:
                Y_zeroed = self.y_samples_unnorm - np.mean(self.y_samples_unnorm)
                Y_std = np.std(self.y_samples_unnorm)
                self.y_samples =( Y_zeroed / Y_std).tolist()
                self.y_err_samples=  (self.y_err_samples_unnorm/Y_std).tolist()
            else:
                self.y_samples = self.y_samples_unnorm
                self.y_err_samples = self.y_err_samples_unnorm
        else:
            self.y_samples = self.y_samples_unnorm
            self.y_err_samples = self.y_err_samples_unnorm
    ...
    def _fit(self):
        if self.dirty:
            ...
            if self.xi == 'auto':
                random_samples = np.array(self._get_random_points(10000))
                random_std = []
                _, err = self.model.predict(random_samples, return_std = True)
                #random_std.append(err)
                random_std = err.copy()
                self.xi_calc = np.abs(np.mean(np.asarray(random_std)**2)/best_val)
            else:
                self.xi_calc = self.xi

            self.Acq = AcquisitionFunctionEI(self.model, best_val, use_efficiency=self.use_efficiency,
                    mean_cutoff=self.mean_cutoff, xi = self.xi_calc, invert = False, explore= self.explore)
    ...
    def _get_random_points(self, N_points):
        ranges = np.array([np.ptp(i) for i in self.bounds])
        offsets = np.array([i[0] for i in self.bounds])
        random_points = [np.random.rand(len(self.bounds)) * ranges + offsets for i in range(N_points)]
